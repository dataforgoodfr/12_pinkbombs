...
def make_geo(input_df, input_id, input_column, input_color_theme):
    geo = px.scatter_geo(input_df, locations=input_id, color=input_column, locationmode="country names",
                               size=input_column,
...
def make_area_chart(input_df, input_x, input_y, input_color_theme):
    ...
    area = px.area(input_df, x="Year", y="Tonnes - live weight", color="Country Name En", line_group="Country Name En")

    bar = px.bar(group_df, x=input_x, y=input_y, color=input_y, color_continuous_scale=input_color_theme, log_y=True)
    # height=300
    return area
